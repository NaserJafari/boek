(function (doc, win, onclick, gid, classname, content, showMessage) {
  var a,
    ...
    start = function () {
      ...
      finished = 0;
      colorLabel[content] = colorLabel[classname] =
        players[(current = (current + 1) % 2)];
      for (a = 1; a < 7; a++)
        for (b = 1; b < 8; b++) cellAt(a, b)[classname] = "";
    },
        ...
        ? (function (i, j) {
            return (
              ...
              (function (i, j) {
                for (
                  a = i - 1, b = j - 1;
                  0 < a && !(1 > b) && isCurrentColor(a, b);
                  a--
                )
                  b--;
                for (
                  c = i + 1, b = j + 1;
                  7 > c && !(7 < b) && isCurrentColor(c, b);
                  c++
                )
                  b++;
                return 4 < c - a;
              ...
              (function (i, j) {
                for (
                  a = i - 1, b = j + 1;
                  0 < a && !(7 < b) && isCurrentColor(a, b);
                  a--
                )
                  b++;
                for (
                  c = i + 1, b = j - 1;
                  7 > c && !(1 > b) && isCurrentColor(c, b);
                  c++
                )
                  b--;
                return 4 < c - a;
          ...
          ? (finished =
              1 &&
              win[showMessage](
                doc[gid](wonLabel)[content].replace(
                  "%s",
                  players[current].toLowerCase()
                )
              ) &&
              start())
          : (colorLabel[content] = colorLabel[classname] =
